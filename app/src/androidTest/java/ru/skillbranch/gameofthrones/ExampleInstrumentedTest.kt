...
class ExampleInstrumentedTest {
    ...
    fun insert_house_and_drop_db() {
        ...
        val lock0 = Object()
        runBlocking {
            RootRepository.insertHouses(ctx, listOf(stubHouseStark)) {
        ...
        runBlocking {
            ...
            RootRepository.isNeedUpdate(ctx) {
                needResult = it
...
//            synchronized(lock) { lock.wait() }
        ...
        val lock0 = Object()
        runBlocking {
            RootRepository.insertHouses(context, listOf(stubHouseStark, stubHouseTargaryen)) {
                synchronized(lock0) { lock0.notify() }
            }
            // synchronized(lock0) { lock0.wait() }
        }
        ...
        runBlocking {
            ...
            RootRepository.insertCharacters(context, characters) {
                synchronized(lock1) { lock1.notify() }
            }
            //synchronized(lock1) { lock1.wait() }
        }
        ...
        runBlocking {
            ...
            RootRepository.findCharactersByHouseName(context, "Stark") {
                actualCharacters = it
...
//            synchronized(lock) { lock.wait() }
        ...
        runBlocking {
            ...
            RootRepository.findCharacterFullById(context, "583") {
                actualCharacter = it
                synchronized(lock2) { lock2.notify() }
            }
//            synchronized(lock2) { lock2.wait() }
