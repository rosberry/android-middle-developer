...
class ShimmerDrawable private constructor(private val pattern: MutableList<Shape>) : Drawable() {
    ...
    private val shimmerAnimator = ValueAnimator.ofFloat(1f)
        .apply {
    ...
    override fun draw(canvas: Canvas) {
        offset = (shaderWidth?.toFloat())?.times(shimmerAnimator.animatedFraction)
            ?: bounds.width()
                .times(shimmerAnimator.animatedFraction)
        shaderMatrix.reset()
    ...
    private fun prepareShader() {
        shimmerPaint.shader = LinearGradient(
                0f, 0f, shaderWidth?.toFloat() ?: bounds.width()
            .toFloat(), 0f,
                shaderColors,
    ...
    class Builder {
        ...
        fun build(): ShimmerDrawable {
            if (itemCount > 0) {
                var topOffset = shapes.lastOrNull()
                    ?.let { it.offset.second + it.height } ?: 0
                ...
                repeat(itemCount.dec()) {
                    ...
                    topOffset = shapes.lastOrNull()!!
                        .let { it.offset.second + it.height }
                }
            }
